...
//     busboy.end(bodyBuffer); // use .end() instead of .write()
//   });
// }
...
//             throw new Error('Missing required fields: productId, rating, or body');
//           }
...
//             const key = `reviews/${productId}/${crypto.randomUUID()}${ext}`;
//             const { data: uploadData, error: uploadErr } = await supabase
//               .storage
//               .from(BUCKET)
//               .upload(key, buffer, { cacheControl: '3600', upsert: false });
//             if (uploadErr) {
